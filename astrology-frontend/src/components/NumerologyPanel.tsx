          ...
          {row.map(cell => (
            <div
              ...
              <span className={[
                ...
                cell.present ? 'text-amber-300' : 'text-white/15',
              ].join(' ')}>
                {cell.number}
              </span>
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 px-5 py-5">
      ...
      <div className="flex flex-wrap gap-2">
        {missing.map(n => (
          <span key={n}
            className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 bg-white/5 font-bold text-white/35">
            {n}
          </span>
        ))}
      </div>
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 overflow-hidden">
      <div className="px-5 pt-5 pb-3">
        <p className="text-xs font-medium uppercase tracking-widest text-white/40">Planes of Expression</p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/10 text-left text-[11px] uppercase tracking-wider text-white/25">
            ...
            {planes.map(p => {
              ...
              return (
                <tr key={p.name} className="border-b border-white/5 hover:bg-white/[0.02]">
                  <td className="px-5 py-2.5 font-medium text-white/80">{p.name}</td>
